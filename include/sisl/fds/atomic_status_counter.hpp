...
#pragma pack(1)
    struct status_counter_t {
        ...
        status_counter_t(const counter_type cnt = 0, const status_type s = default_val) : counter{cnt}, status{s} {}

        explicit operator uint64_t() const { return to_integer(); }
...
public:
    using counter_type = typename status_counter_t::counter_type;
    using status_type = typename status_counter_t::status_type;
     ...
     * @return StatusType
     */
    [[nodiscard]] status_type get_status() const { return m_val.load(std::memory_order_acquire).status; }
     ...
     * @return int32_t
     */
    [[nodiscard]] counter_type count() const { return m_val.load(std::memory_order_acquire).counter; }

    /**
     * @brief Get the status count object atomically
     *
     * @return std::pair< StatusType, int32_t >: A pair of status and counter
     */
    [[nodiscard]] std::pair< status_type, counter_type > get_status_count() const {
        const auto val{m_val.load(std::memory_order_acquire)};
        return std::pair{val.status, val.counter};
    }
     ...
     */
    [[nodiscard]] bool dec_xchng_status_ifz(const status_type exp_status, const status_type new_status) {
     ...
     */
    [[nodiscard]] bool dec_xchng_status_only_ifz(const status_type exp_status, const status_type new_status) {
        const auto new_val{set_value([exp_status, new_status](status_counter_t& val) {
     ...
     */
    [[nodiscard]] bool increment_if_status(const status_type exp_status) {
     ...
     */
    [[nodiscard]] bool decrement_testz_and_test_status(const status_type exp_status) {
     ...
     */
    [[nodiscard]] bool dec_set_status_ifz(const status_type new_status) {
     ...
     */
    [[nodiscard]] bool decrement_testz(const counter_type count = 1) {
     ...
     */
    [[nodiscard]] bool set_atomic_value(const auto& modifier) {
...
private:
    status_counter_t set_value(const auto& modifier) {
        status_counter_t old_v, new_v;
        do {
            old_v = m_val.load(std::memory_order_acquire);
            new_v = old_v;
