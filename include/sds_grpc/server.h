...
class ServerCallData final : public BaseServerCallData {
    ...
    using request_call_func_t = std::function< void(TSERVICE*, ::grpc::ServerContext*, TREQUEST*,
                                                    ::grpc::ServerAsyncResponseWriter< TRESPONSE >*,
                                                    ::grpc::CompletionQueue*, ::grpc::ServerCompletionQueue*, void*) >;

    using handle_call_func_t = std::function<::grpc::Status(TREQUEST&, TRESPONSE&) >;
...
private:
    ...
    ServerCallData(TSERVICE* service, ::grpc::ServerCompletionQueue* cq, request_call_func_t wait_request,
...
protected:
    ...
    void do_create() { wait_request_func_(service_, &context_, &request_, &responder_, cq_, cq_, this); }

    void do_process() {
        (new T(service_, cq_, wait_request_func_, handle_request_func_))->proceed();
...
class GrpcServer : private boost::noncopyable {
    ...
    enum State { VOID, INITED, RUNNING, SHUTTING_DOWN, TERMINATED };

private:
    GrpcServer();

    bool init(const std::string& listen_addr, uint32_t threads, const std::string& ssl_key,
...
public:
    ...
    bool is_terminated() { return state_ == State::TERMINATED; }

    ::grpc::ServerCompletionQueue* completion_queue() { return cq_.get(); }

    template < typename TSVC >
    bool register_async_service() {

        BOOST_ASSERT_MSG(State::INITED == state_, "register service in non-INITED state");

        auto name = TSVC::service_full_name();

        BOOST_ASSERT_MSG(services_.find(name) == services_.end(), "Double register async service.");
    ...
    bool register_rpc(std::function< void(typename TSVC::AsyncService*, ::grpc::ServerContext*, TREQUEST*,
        ...
        BOOST_ASSERT_MSG(State::RUNNING == state_, "register service in non-INITED state");
        ...
        (new ServerCallData< typename TSVC::AsyncService, TREQUEST, TRESPONSE >(svc, cq_.get(), request_call_func,
                                                                                handle_request_func))
            ->proceed();
