...
#define SETTINGS_SCHEMA_INIT(settings_type, schema_name)                                                               \
    sisl::SettingsFactory< settings_type##T >::instance().m_raw_schema =                                               \
        std::string((const char*)&schema_name##_fbs, (size_t)schema_name##_fbs_len);
#endif
...
public:
    ...
    void load_file(const std::string& config_file_path) {
...
private:
    bool check_restart_needed(const SettingsT* new_settings, const SettingsT* current_settings) {
        ...
        old_parser.builder_.Finish(SettingsT::TableType::Pack(old_parser.builder_, current_settings, nullptr));
