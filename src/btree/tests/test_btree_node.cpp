...
struct NodeTest : public testing::Test {
    ...
    void validate_get_any(uint32_t start, uint32_t end) const {
        ...
        V out_v;
        auto result =
            m_node1->get_any(BtreeKeyRangeSafe< K >{start_key, true, end_key, true}, &out_k, &out_v, true, true);
        if (result.first) {
            validate_data(out_k, out_v);
        } else {
            result =
                m_node2->get_any(BtreeKeyRangeSafe< K >{start_key, true, end_key, true}, &out_k, &out_v, true, true);
            if (result.first) {
                validate_data(out_k, out_v);
            } else {
