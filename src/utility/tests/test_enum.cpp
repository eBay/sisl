...
class Container {
public:
    SCOPED_ENUM_DECL(signed_enum, int16_t)
};

SCOPED_ENUM_DEF(Container, signed_enum, int16_t, val1, val2)
TEST_F(EnumTest, scoped_enum) {
    auto enum_lambda{[](const Container::signed_enum& val) {
        switch (val) {
        case Container::signed_enum::val1:
            return 1;
        case Container::signed_enum::val2:
            return 2;
        default:
            return 0;
        };
    ...
    ASSERT_EQ(enum_lambda(Container::signed_enum::val1), 1);
