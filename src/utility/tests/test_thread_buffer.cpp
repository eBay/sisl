...
protected:
    ...
    static void scrapper_thread(ThreadBufferTest* test) {
        ...
        do {
            usleep(50000);
            test->m_buffer.access_all_threads([&merge_list](MyList* ml, bool is_thread_running, bool is_last_thread) {
                merge_list.add(*ml);
                return true;
            });
            ...
            std::cout << "Scrapped " << merge_list.m_list.size()
                      << " in this iteration. total_entries_recvd so far = " << total_entries_recvd << "\n";
