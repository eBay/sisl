...
private:
    K m_base_key;
    folly::small_vector< value_info, 8, kcount_t > m_values;

public:
    std::pair< V&, bool > try_insert(const koffset_range_t range, const V& value) {}

    uint32_t find(const koffset_range_t range, std::vector< const value_info* >& out_values) const {
        uint32_t count{0};
        // First binary_search for the location, if there is a valid
        auto [idx, found] = binary_search(-1, static_cast< int >(num_entries()), range.first);
        while (found) {
            out_values.push_back(m_values[idx]);
            ++count;

            if (++idx == m_values.size()) break;

            // Check if upper bound of the input_range is within bounds of next entry.
            found = (m_values[idx]->compare_range(range.second) >= 0);
        }
        return count;
    }
