...
public:
    ...
    void set(bool value, uint64_t start_bit, uint32_t total_bits = 1) {
    ...
    sisl::BitBlock get_next_contiguous_n_reset_bits(uint64_t start_bit, uint32_t n) {
        sisl::BitBlock ret{boost::dynamic_bitset<>::npos, 0u};

        std::unique_lock l(m_mutex);
        // LOGINFO("ShadowBitset next reset request bit={}", start_bit);
        start_bit = (start_bit == 0) ? (~m_bitset).find_first() : (~m_bitset).find_next(start_bit - 1);
        for (auto b = start_bit; b < m_bitset.size(); ++b) {
            // LOGINFO("ShadowBitset next reset bit = {}, m_bitset[b]={}", start_bit, m_bitset[start_bit]);
            if (!m_bitset[b]) {
                if (ret.nbits == 0) {
                    ret.start_bit = b;
                    ret.nbits = 1;
...
private:
    void set_or_reset(uint64_t bit, bool value) {
        std::unique_lock l(m_mutex);
        m_bitset[bit] = value;
    }
...
protected:
    ...
    void set(uint64_t start_bit, uint32_t nbits) {
        m_bset.set_bits(start_bit, nbits);
        m_shadow_bm.set(true, start_bit, nbits);
    }

    void reset(uint64_t start_bit, uint32_t nbits) {
    ...
    void validate_by_next_bits(bool by_set = true) {
...
void run_parallel(uint64_t total_bits, uint32_t nthreads, const std::function< void(uint64_t, uint32_t) >& thr_fn) {
    uint64_t start = 0;
    auto n_per_thread = std::ceil((double)total_bits / nthreads);
    std::vector< std::thread* > threads;

    while (start < total_bits) {
        threads.push_back(
            new std::thread(thr_fn, start, std::min((uint32_t)n_per_thread, (uint32_t)(total_bits - start))));
        start += n_per_thread;
    }
