...
-----------------------------------------------------------------------------*/
#ifndef __HASH_PMURHASH_HPP__
#define __HASH_PMURHASH_HPP__
namespace fds {

//#include "libutils/fds/hash/PMurHash.h"
...
#define MURMUR_CODE1 (0xcc9e2d51)
#define MURMUR_CODE2 (0x1b873593)
...
static void PMurHash32_Process(uint32_t* ph1, uint32_t* pcarry, const void* key, int len) {
    uint32_t h1 = *ph1;
    uint32_t c = *pcarry;
    ...
    int i = (4 - n) & 3;
    if (i && i <= len) { DOBYTES(i, h1, c, n, ptr, len); }
...
static uint32_t PMurHash32_Result(uint32_t h, uint32_t carry, uint32_t total_length) {
    uint32_t k1;
    int n = carry & 3;
    if (n) {
        k1 = carry >> (4 - n) * 8;
...
static uint32_t PMurHash32(uint32_t seed, const void* key, int len) {
...
static void PMurHash32_test(const void* key, int len, uint32_t seed, void* out) {
    uint32_t h1 = seed, carry = 0;
    const uint8_t* ptr = (uint8_t*)key;
