...
#define INVALID_CURSOR boost::dynamic_bitset<>::npos
  ...
  public:
    ...
    uint32_t attach() {
        ...
        LOG(INFO) << "ThreadRegistry attached new thread,"
                  << " assigned thread_num = " << thread_num
                  << " thread_name = " << thread_name;
        urcu_ctl::register_rcu();
        return thread_num;
    }

    void detach(uint32_t thread_num) {
        LOG(INFO) << "ThreadRegistry detaching thread_num = " << thread_num;
    ...
    void access_all_threads(std::function<void(T*)> cb) {
        thread_registry->for_all(
            [this, cb](uint32_t thread_num, bool is_thread_exited) {
                if(m_buffers[thread_num]) cb(m_buffers[thread_num].get());
            });
    }
